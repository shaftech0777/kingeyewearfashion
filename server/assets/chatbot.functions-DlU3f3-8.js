import { T as TSS_SERVER_FUNCTION, c as createServerFn } from "./server-Dznb2wd8.js";
...
const askKingBot_createServerFn_handler = createServerRpc({
  id: "4a9e4e225d88c2cfb9e67c2effb2466038d3dfd47dfbf2d8325249baf2b2404c",
  name: "askKingBot",
  filename: "src/lib/chatbot.functions.ts"
}, (opts) => askKingBot.__executeServer(opts));
const askKingBot = createServerFn({
  method: "POST"
}).inputValidator((d) => d).handler(askKingBot_createServerFn_handler, async ({
  data
}) => {
  ...
  return {
    text: j.choices?.[0]?.message?.content ?? "Sorry, no response."
  };
});
export {
  askKingBot_createServerFn_handler
};
